...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    im_(F.im_.clone()),rgb_(F.rgb_.clone()),semidense_flag_(false),semidense_begin_inter(false),
        SemiDenseMatrix(im_.rows, std::vector<ProbabilityMapping:: depthHo>(im_.cols, ProbabilityMapping::depthHo()) ),mnFrameId(F.mnId),  mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
    ...
    mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb/2), mpMap(pMap)
{
    ...
    //  compute some value for  semi dense  process
     ...
     gradx.release();
     grady.release();
       depth_map_first=cv::Mat::zeros(im_.rows, im_.cols, CV_32F);
       depth_sigma_first=cv::Mat::zeros(im_.rows,im_.cols,CV_32F);
      depth_map_ = cv::Mat::zeros(im_.rows, im_.cols, CV_32F);
      depth_map_finally = cv::Mat::zeros(im_.rows, im_.cols, CV_32F);
