...
float ProbabilityMapping::bilinear1(const cv::Mat& img, const float& y, const float& x)//求插入的像素的灰度值
{
    int x0 = (int)std::floor(x);//整数部分
    int y0 = (int )std::floor(y);
    int x1 = x0 + 1;
    int y1 =  y0 + 1;//整数部分加1

    float x0_weight = x1 - x;//小数部分被1减
    float y0_weight = y1 - y;
    float x1_weight = 1.0f - x0_weight;//小数部分
    float y1_weight = 1.0f - y0_weight;
   uchar* d=&img.data[y0*img.step+x0];//uchar表示的范围是0-255，step表示每一行所占的字节的数目
...
void ProbabilityMapping::SemiDenseLoop(){
  ...
  for(size_t i =0;i < vpKFs.size(); i++ )
  {
      ...
      if(kf->isBad() || kf->semidense_flag_||kf->semidense_begin_inter)
        continue;
        std::vector<ORB_SLAM2::KeyFrame*> closestMatches = kf->GetBestCovisibilityKeyFrames(covisN);//找到与当前帧最匹配的N个关键帧放在容器closesMatches中
...
****************************************************/
        cv::Mat image = kf->GetImage();//这里的KeyFrame就是关键帧容器中的
       //cv::Mat image_debug = image.clone();

        int number=0;
        for(int y = 0+2; y < image.rows-2; y++)//y+=2 x+=2;
        {
          for(int x = 0+2; x< image.cols-2; x++)//相当于跳行检测,可以加快速度
          {
             //  if(kf->GradImg.ptr<float>(y)[x]<lambdaG){continue;}
              if(kf->GradImg.at<float>(y,x) < lambdaG){continue;}
              float pixel=(float)kf->im_.ptr<uchar>(y)[x];//像素坐标
              float t_pi=kf->GradTheta.ptr<float>(y)[x];
            //float pixel =(float) image.at<uchar>(y,x); //maybe it should be cv::Mat
          // cout<<"(x,y)="<<x<<","<<y<<endl;
            std::vector<depthHo> depth_ho;// struct depthHo是个结构，里面包含depth 和sigma 还有一个bool型变量supported
           depth_ho.clear();
                 for(size_t j=0;j<closestMatches.size(); j++)
            {
                cv::Mat F12 = F[j];
              ...
              if(dh_temp.supported&&dh_temp.depth>0.01)//&&dh_temp.sigma/dh_temp.depth<0.01)
               {
                ...
                kf->depth_sigma_.at<float>(y,x) = dh_temp.sigma;
                  number+=1000;//这里是不是有点问题　？？？在这里要不要进行个约束？除掉之前匹配不太合适的点
...
for(auto &i:orb_depths){i=1/i;}//c++第五版的内容 意思是引用std::vector<float> orb_depths中的每个数据即深度 ，然后求深度的逆
  float sum = std::accumulate(orb_depths.begin(), orb_depths.end(), 0.0);
  float mean = sum / orb_depths.size();

  std::vector<float> diff(orb_depths.size());//定义了大小是orb_depths.size()的名字是diff的容器
  std::transform(orb_depths.begin(), orb_depths.end(), diff.begin(), std::bind2nd(std::minus<float>(), mean));
  // bind2nd 可以用于将二元算子(binary functor, bf)转换为一元算子(unary functor, uf)。转换过程需要二个参数：bf与值(v)。
   //bf( x, v) - 用于bind2nd函数 V是常量 将某一范围内(如容器中)的各个值与一基准值相比较时尤其有用。将所有元素都与mean相减
...
ustar_var=1;
 ...
 /*
  void ProbabilityMapping::EpipolarSearch_afterzy(ORB_SLAM2::KeyFrame* kf1, ORB_SLAM2::KeyFrame *kf2, const int x, const int y, float pixel,
    float min_invdepth, float max_invdepth, depthHo *dh,cv::Mat F12,float&pjju,float& pjjv,float& best_u,float& best_v,float th_pi,float&a,float&b,float&c)
{

   a = x*F12.at<float>(0,0)+y*F12.at<float>(1,0)+F12.at<float>(2,0);
   b = x*F12.at<float>(0,1)+y*F12.at<float>(1,1)+F12.at<float>(2,1);
   c = x*F12.at<float>(0,2)+y*F12.at<float>(1,2)+F12.at<float>(2,2);//求极线　u=ax+by+c
  ...
  cv::Mat xy1=(cv::Mat_<float>(3,1) << x, y,1.0);//这里的xy 相当于一帧图片的px py
  cv::Mat k2t21=K2*t21;
  cv::Mat Rxy1=K2*R21*K1.inv()*xy1;
  cv::Mat normal1=(cv::Mat_<float>(1,3)<<0,0,1);//相当于平面法向量
  cv::Mat H1=K2*R21*K1.inv();//熟悉一下H的计算方法
  ...
  photo_kf1.clear();
  int halfpath_size=2;
  if(x<halfpath_size||x+halfpath_size>kf1->im_.cols||y-halfpath_size<0||y+halfpath_size>kf1->im_.rows)
  {return;}
  //std::vector<float> w_pq;
  //float num=0;
  //std::vector<int> vec_number;
  std::vector <pair<float,float>> vec_xy;
  ...
  for(int  j=-halfpath_size;j<halfpath_size+1;++j)
  {
    ...
    {
        std::pair<float,float> xy=std::make_pair(i+x,j+y);
        vec_xy.push_back(xy);//局部范围内每个像素的坐标
        cv::Mat xppjj=(cv::Mat_<float>(3,1)<<i+x,j+y,1);
        xppjjzy =(cv::Mat_<float>(2,1)<<i+x,j+y);
        n_point.push_back(xppjj);
        photo_kf1.push_back(bilinear1(kf1->im_,j+y,i+x));//计算出局部的像素的灰度值
        sum_photo_kf1+=bilinear1(kf1->im_,j+y,i+x);//计算出一个（５×５）局部搜索半径内的灰度值的sum

    }
  }
  sum_photo_kf1/=photo_kf1.size();//插入的平均灰度值,最中心的那个灰度值
  ...
  GetSearchRange(umin,umax,x,y,min_invdepth,max_invdepth,kf1,kf2);
   ...
   /*********************************************************************************************************************
     if(kf1->depth_map_first.ptr<float>(y)[x])//<float>(y)[x]就是<float>(y,x);depth_map_firs是一个Mat类型
  {
      //这个条件就是判断最大最小的逆深度值

      cout<<"kf1->depth_map_first.ptr<float>(y)[x]"<<kf1->depth_map_first.ptr<float>(y)[x]<<endl;
  ...
  if(umin==umax){return;}
   ...
   for(int uj = std::floor(umin); uj < std::ceil(umax)+1; ++uj)//在极线上判断匹配最好的点
   {
          vj =-((a/b)*uj+(c/b));
          if(uj<halfpath_size||uj+halfpath_size>kf1->im_.cols||vj-halfpath_size<0||vj+halfpath_size>kf1->im_.rows)
          {continue;}
          if(vj<0|| vj > kf2->im_.rows )
          {continue;}
            // condition 1:
            //if( kf2->GradImg.at<float>(vj,uj) < lambdaG){continue;}
          if(bilinear<float>(kf2->GradImg,vj,uj)<lambdaG)
          {continue;}
            // condition 2:
          float th_epipolar_line = cv::fastAtan2(-a/b,1);
          float temp_gradth =  bilinear<float>(kf2->GradTheta,vj,uj) ;
          ...
          if( temp_gradth > 90 &&  temp_gradth<=270)
              temp_gradth =  temp_gradth - 180;
          if(th_epipolar_line>270)
              th_epipolar_line = th_epipolar_line - 360;
          if(th_epipolar_line>90&&th_epipolar_line<=270)
              {th_epipolar_line=th_epipolar_line-180;}
          if(abs(abs(temp_gradth - th_epipolar_line- 180)) < 80 )
              {continue;}
               // condition 3:
          if(abs(bilinear<float>( kf2->GradTheta,vj,uj) -  th_pi ) > lambdaTheta)
              {continue;}//GradTheta是像素梯度角度  lambdaTheta=45度
              //v::phase(gradx,grady,GradTheta,true)
   ...
   /****************************************************************************************************************
        //为了得到对应的深度float s_depth
              ...
              cv::Mat xn2;
              cv::Mat InverseOrderKf2;
              GetXp(K1, x, y, &xn1);
              GetXp(K2, uj, vj, &xn2);//这个地方出错了！！！！！！
 //********************************卷积********************************************************************************
              cv::Mat A(4,4,CV_32F);
              ...
              float s_depth= R1w.row(2).dot(x3Dt)+t1w.at<float>(2);//所长说ＲＴ是之前跟踪得到的
              ...
              vector<pair<float,float>> vec_uv,InverseKf2,juanjiD;
              ...
              for(size_t i=0;i<n_point.size();++i)
              {
                ...
                kf2zy= (cv::Mat_<float>(2,1) << u_pjj1, v_pjj1);
                std::pair<float,float> uv=std::make_pair(u_pjj1,v_pjj1);
                vec_uv.push_back(uv);
             ...
             {
             pjjfenzhi+=(photo_kf1[i]-sum_photo_kf1)*(photo_kf2[i]-sum_photo_kf2);//可以通过卷积的得到
    ...
    /*********************************************需要调整**********************************************************************
      ...
      //求kf2的逆序
             vector<pair<float,float>>*p=&photo_kf2;
            for(int i=0;i<p->size();i++)
            {
           // InverseKf2 =(*p)[(p->size())-i];
            InverseKf2.push_back((*p)[(p->size())-i]);
            }



           for (int i=0;i<kf2zy.rows;i++)
             {
                 for (int j=0;j<kf2zy.cols;j++)
                 {

                     kf2zy.at<float>(i,j)=kf2zy.at<float>((kf2zy.rows -i),(kf2zy.cols -j));

                 }


             }
          cv::filter2D(xppjjzy,kf2zy,&Km,cvPoint(-1,-1));

          cv::Mat output_temp=kf2zy; //卷积求法
           output_temp=xppjjzy.mul(kf2zy);

           for (int i=0;i<xppjjzy.rows;i++)

            {
                for(int j=0;j<xppjjzy.cols;j++)
                {
                for(int m=0;m<kf2zy.rows+1;++m)
                    {
                        for(int n=j;n<j+kf2zy.cols+1;++n)
                        {
                            output_temp.at<float>(m,n)=output_temp.at<float>(m,n)+xppjjzy.at<float>(i,j)*kf2zy.at<float>((m-i),(n-j));
                        }
                    }
            }
            }
          ...
          float max=0;
           for(int i=0;i<output_temp.rows;i++)
            {
                for (int j=0;j<output_temp.cols;j++)
                  {
                    //output_temp.at<float>(i,j)+=output_temp.at<float>(i,j);

                    output_temp.at<float>(i,j)+=output_temp.at<float>(i,j) /(std::sqrt(pjjfenmu1*pjjfenmu2));
                   if(max<output_temp.at<float>(i,j)&&output_temp.at<float>(i,j)<=1)//这里的应该是要sort的，然后找到最大的值
                         {
                             max=output_temp.at<float>(i,j);
                         }
                   best_pixel=max;
                   best_u =max;
                   best_v =  -( (a/b)*best_u + (c/b) );
...
/*********************************************************************************************************************
      ...
      vector<pair<float,float>>*p1=&photo_kf1;
            vector<pair<float,float>>*p2=&InverseKf2;
            for(int i=0;i<p1->size();i++)
            for (int j=0;j<p2->size();j++)
                 {
                  juanjiD+=(*p1)[i]+(*p2)[j];
                 juanjiD.push_back(juanjiD);
             ...
             int zyii=xn2.cols;
             cv::Mat juanjiD;
             for(zyj=0;zyj< zyjj;zyj++)
             {for(zyi=0;zyi<zyii;zyi++)
                 {InverseOrderKf2.at<float>((zyjj-zyj-1),(zyii-zyi-1)) = xn2.at<float>(zyj,zyi);
                   std::cout<<"qiuni"<<std::endl;
                 }
           }

          //求InverseOrderKf2与xn1的卷积  juanjiD××××××××××××××××××××××××××××××××××××××××××××××
             for(int i=0;i<InverseOrderKf2.rows;i++)
                 {
                     for(int j=0;j<InverseOrderKf2.cols;j++)
                     {
                         for(int m=0;m<i+xn1.rows;m++)
                         {
                             for(int n=0;n<j+xn1.cols;n++)
                             {
                                 juanjiD.at<float>(m,n)=(juanjiD.at<float>(m,n)+InverseOrderKf2.at<float>(i,j)*xn1.at<float>(m-i,n-j))/ std::sqrt(pjjfenmu1*pjjfenmu2);

                                 if(juanjiD.at<float>(m,n)=1)
                                 {

                                 best_pixel=juanjiD.at<float>(m,n);
                                 best_u =juanjiD.at<float>(m,n);
                                 best_v =  -( (a/b)*best_u + (c/b) );
                                 }
                             }
           ...
           //  for(int j=0;j<juanjiD.rows;j++)
          //   for(int j1=0;j1<juanjiD.cols;j1++)
          //       {

         //          juanjiD.at<float>(j,j1)+= ( juanjiD.at<float>(j,j1))/ std::sqrt(pjjfenmu1*pjjfenmu2)  ;
         //          if (juanjiD.at<float>(j,j1)>=1)
         //            {

         //                best_u =juanjiD.at<float>(j);
         //                best_v =  -( (a/b)*best_u + (c/b) );
         //             }
                //best_pixel = best_u;
  ...
  //******************************************************************************************************************/
    ...
    //float s_depth=(k2t21.at<float>(0,0)-k2t21.at<float>(2,0)*uj)/(Rxy1.at<float>(2,0)*uj-Rxy1.at<float>(0,0)+1e-10);//求在另一关键帧中的深度
...
if(ncc_score>0.85){
    pjju=best_pixel;
    pjjv=-((a/b)*pjju+(c/b));
    best_photometric_err = pixel - bilinear<uchar>(kf2->im_,-((a/b)* pjju+(c/b)), pjju);
    best_gradient_modulo_err= kf1->GradImg.at<float>(y,x)  - bilinear<float>( kf2->GradImg,-((a/b)* pjju+(c/b)), pjju);
      uj_plus =  pjju + 1;
      vj_plus = -((a/b)*uj_plus + (c/b));
      uj_minus =  pjju - 1;
     vj_minus = -((a/b)*uj_minus + (c/b));
    ...
    if(vj_plus<0||vj_minus<0||vj_plus>kf2->im_.rows||uj_minus>kf2->im_.cols)   //  if abs(a/b) is large,   a little step for uj, may produce a large change on vj. so there is a bug !!!  vj_plus may <0
      ...
      denomiator = (g*g + (1/THETA)*q*q);
    ustar =  pjju + (g*best_photometric_err + (1/THETA)*q*best_gradient_modulo_err)/denomiator;
...
float sum_photo_kf1=0.0,sum_gradient_kf1=0.0;
  ...
  float best_gradient_modulo_err = 0.0;
  float NSSD=1000000;
  float ncc_score=0.0,second_score=0.0,ncc_scorezy=0.95;
   ...
   for(int uj = std::floor(umin); uj < std::ceil(umax)+1; ++uj){
         ...
         //     if(number_depth>number_h-1)number_depth=number_h-1;
           float s_depth=(k2t21.at<float>(0,0)-k2t21.at<float>(2,0)*uj)/(Rxy1.at<float>(2,0)*uj-Rxy1.at<float>(0,0)+1e-10);
            ...
            std::vector<float> photo_kf2,photo_kf2_,gradient_kf2,Ivskf2;
           vector<int> vec_number1;
               photo_kf2.clear();
             if(n_point.size()==0){continue;}
               std::vector<float>w1_pq;
              ...
              //  cout<<"pjj_="<<pjj_<<"pjj_t="<<pjj_t<<endl;
                ...
                float sum_photo_kf2=0.0,sum_gradient_kf2=0.0,num_ramuda=0,sum_photo_kf;
               for(size_t i=0;i<n_point.size();++i){

                cv::Mat uvpjj=H*n_point[i];
             float u_pjj1=uvpjj.at<float>(0,0)/uvpjj.at<float>(2,0);
             float v_pjj1=uvpjj.at<float>(1,0)/uvpjj.at<float>(2,0);
            // vec_uv=std::make_pair(u_pjj1,v_pjj1);
             photo_kf2.push_back(bilinear1(kf2->im_,v_pjj1,u_pjj1));
             sum_photo_kf2+=bilinear1(kf2->im_,v_pjj1,u_pjj1);
         ...
         // pjjfenzhi+=(photo_kf1[i]-sum_photo_kf1)*(photo_kf2[i]-sum_photo_kf2);
          pjjfenmu1+=pow((photo_kf1[i]-sum_photo_kf1),2);
           ...
           // std::reverse(std::begin(photo_kf2), std::end(photo_kf2));
            int m=photo_kf1.size();
            int n=photo_kf2.size();
            int i,j;
                int k=m+n-1;//卷积后数组长度
                float c[k];
                ...
                for(i=0; i<k; i++)
                {
                    for(j=std::max(0,i+1-n); j<=std::min(i,m-1); j++)

                        c[i]+=photo_kf1[j]*photo_kf1[i-j]; //卷积

                }

                 for(i=0; i<k; i++)
                 {
                      c[i]=c[i]/(std::sqrt(pjjfenmu1*pjjfenmu2));
        ...
        //     for(int  i=0;i<k;i++)
           //  {
             //sum_photo_kf += (photo_kf1[i])*(photo_kf2[i]);//卷积=r1*r1+r2*r2+r3*r3.....类型不匹配
            // }
            //sum_photo_kf=sum_photo_kf/(std::sqrt(pjjfenmu1*pjjfenmu2));//从这里开始有问题了
        //            if(sum_photo_kf>ncc_scorezy)
          //  {
            //        second_pixel=best_pixel;
              //      second_score=ncc_scorezy;
                //    best_pixel = uj;
                  // ncc_score=sum_photo_kf;
               //}
                                        ...
                                        ORB_SLAM2::KeyFrame* kf,ORB_SLAM2::KeyFrame* kf2)
{
  ...
  cv::Mat xp2_max = R21*xp1/mininvd+t21;//相当于乘以深度得到了深度的范围？？？？？？
